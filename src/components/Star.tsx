...
export function StarRating({ stars }: props) {
  ...
  function mouseOver(id: number): void {
    setStar((star) =>
      star.map((item, itemId: number) => {
        if (itemId <= id) {
          return `${item} stars`;
        }
        return item;
  ...
  function mouseOut(id: number): void {
    setStar((star) =>
      star.map((item, itemId) => {
        return item.split(" ")[0];
      })
    );
  }
    ...
    <>
      {star.map((item: string, id: number) => {
        return (
          <svg
            className={
              star.some((item) => item === "clicked stars")
                ? item === "clicked stars"
                  ? "clicked"
                  : ""
                : item
